...
def get_dentist_info(name: str = "", specialist: str = "", procedure: str = "") -> str:
    db, cursor = get_db_cursor()
    if not db or not cursor:
        return {"error": "Failed to connect to the database"}

    try:
        # Chamada de procedimento armazenado com parâmetros como tupla
        cursor.callproc('sp_get_dentist_info', (name, specialist, procedure))
        result = []
...
def get_specialist(arg: str) -> str:
    db, cursor = get_db_cursor()
    if not db or not cursor:
        return {"error": "Failed to connect to the database"}
...
def get_all_dentist(arg: str) -> str:
    db, cursor = get_db_cursor()
    if not db or not cursor:
        return {"error": "Failed to connect to the database"}
...
def get_procedures(specialist: str) -> str:
    db, cursor = get_db_cursor()
    if not db or not cursor:
        return {"error": "Failed to connect to the database"}

    try:
        # Chamada de procedimento armazenado com um único parâmetro como tupla
        cursor.callproc('sp_get_procedures_by_specialty', (specialist,))
        result = []
        for res in cursor.stored_results():
            result.extend(res.fetchall())
            print(result)
